...
class PagesXmlReader {
	...
	private static Iterator<PageInfo> getPagesIterator(String document) throws XMLStreamException, FactoryConfigurationError{
		final XMLStreamReader xmlReader = XMLInputFactory.newInstance().createXMLStreamReader(new	ByteArrayInputStream(document.getBytes()));
		    	...
		    	try {
					while(xmlReader.hasNext()){
						...
						case XMLStreamConstants.START_ELEMENT:
							...
							else if(ElementNames.TEXT_ELEMENT_NAME.equals(currentEltName) && currentPage.getNextPage() == null){
								currentPage.setNextPage(WikipediaPageUtils.getNextPage(xmlReader.getElementText()));
							}
						...
						case XMLStreamConstants.END_ELEMENT:
							currentEltName = null;
							if(xmlReader.getLocalName().equals(ElementNames.PAGE_ELEMENT_NAME) && currentPage != null){
								return true;
							}
...
/*	public static void readPagesFromXml(String document, Context context) {
		...
		String currentEltName = null;
		
		try {
			XMLStreamReader xmlReader = XMLInputFactory.newInstance().createXMLStreamReader(new	ByteArrayInputStream(document.getBytes()));
			...
			while(xmlReader.hasNext()){
				...
				case XMLStreamConstants.CHARACTERS:
					if(ElementNames.TITLE_ELEMENT_NAME.equals(currentEltName))
						currentPage.setTitle(xmlReader.getText());
					else if(ElementNames.TEXT_ELEMENT_NAME.equals(currentEltName) && currentPage.getNextPage() == null)
						currentPage.setNextPage(WikipediaPageUtils.getNextPage(xmlReader.getTextCharacters()));
