...
public class NextPageGetter {
	...
	if (tag.equals("<") && cursorIndex + 1 < chars.length
		&& chars[cursorIndex + 1] == '/')
	    return false;
	...
	for (cursorIndex = 0; cursorIndex < chars.length - 1; cursorIndex++) {
		...
		if (closingTagsToFind.isResultTag()
			&& potentialClosingTagToFind.equals("]]")) {
	    ...
	    else if (!closingTagsToFind.isEmpty()
		    && isCursorAtTag(closingTagsToFind.tagName)) {
		closingTagsToFind.remove();
	    }
	}
